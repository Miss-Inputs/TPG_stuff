...
This is not used as much anymore, could be named better, it is mainly there to combine multiple seasons into one.
"""
...
def main() -> None:
	...
	season_args.add_argument(
		'--start-round',
		type=int,
		help='Number to start counting rounds from, defaults to 1',
		default=1,
	)
	...
	if season_starts or season:
		for season, season_rounds in groupby(sorted(rounds, key=_season_sorter), _season_sorter):
			...
			print(
				f'Season {season}: {len(season_rounds)} rounds: {[r.display_name for r in season_rounds]}'
			)
